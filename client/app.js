...
try {
    ...
    console.error("Supabase gagal dimuat.", e);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (roomFromUrl) {
        ...
        if (savedName) {
            ...
            if (btn) btn.innerText = "Mencoba Masuk (Auto-Join)...";
...
window.exitTutorial = function() {
    if(!confirm("Keluar dari Mode Latihan?")) return;
    window.isTutorialMode = false;
    window.gameStarted = false;
...
window.joinRoom = async function() {
    if (!db) return alert("Koneksi ke database gagal.");
    try {
        const roomInput = document.getElementById('room-input');
        const nameInput = document.getElementById('name-input');
        ...
        if(lobbyBtn) lobbyBtn.innerText = "Memuat data...";
        ...
        if (fetchError && fetchError.code !== 'PGRST116') {
            ...
            return alert("Terdapat kendala jaringan.");
        }
...
window.resetGame = async function() {
    if(confirm("Memulai ulang permainan?")) {
        const initialBoard = [7,7,7,7,7,7,7, 0, 7,7,7,7,7,7,7, 0];
    ...
    html2canvas(area, { backgroundColor: '#fef3c7', scale: 2 }).then(canvas => {
        canvas.toBlob(blob => {
            ...
            if (navigator.canShare && navigator.canShare({ files: [file] })) {
                ...
                alert("Gambar berhasil di-unduh! Silakan bagikan secara manual.");
            }
...
function renderBoard() {
    if (window.isTutorialMode) {
        document.getElementById('status-panel').innerText = (gameState.current_player === 1) ? "Giliran Pemain Bawah" : "Giliran Pemain Atas";
        document.getElementById('status-panel').className = "text-lg md:text-xl font-bold text-blue-900 bg-blue-100 px-6 py-2 rounded-full shadow-sm";
        renderGameHoles();
        return;
    }
...
function renderGameHoles() {
    ...
    if (myRole === 1 || myRole === null || window.isTutorialMode) {
...
function createHoleHTML(index, count) {
    ...
    if (window.isTutorialMode) {
        ...
        const isP2Turn = gameState.current_player === 2;
        const isP1ZoneLocal = index >= 0 && index <= 6;
        const isP2ZoneLocal = index >= 8 && index <= 14;
        canClick = count > 0 && !gameState.game_over && !isAnimating && ((isP1Turn && isP1ZoneLocal) || (isP2Turn && isP2ZoneLocal));
    } else {
        const isMyTurn = gameState.current_player === myRole;
        const isMyZone = (myRole === 1 && index >= 0 && index <= 6) || (myRole === 2 && index >= 8 && index <= 14);
        canClick = isMyTurn && isMyZone && count > 0 && !gameState.game_over && !isAnimating && window.gameStarted;
    }
...
window.clickHole = async function(holeIndex) {
    ...
    while (isSowing) {
        ...
        while (seeds > 0) {
            ...
            if (!window.isTutorialMode && myChannel) {
    ...
    if (window.isTutorialMode) {
        ...
        gameState.game_over = isGameOver;
        gameState.winner = winner;
        renderBoard();
        if (isGameOver) {
            document.getElementById('status-panel').innerText = "Latihan Selesai!";
        }
    } else {
        if (myChannel) myChannel.send({ type: 'broadcast', event: 'sowing_end' });
